...
$AzADGroup = 'IDH-AirflowPlatform-Admins'


# :run script
Temp/Access_group_review/Delete_RoleAssignment_mass.ps1 -r $Role -s $Scopes -g $AzADGroup
...
process {
    ...
    if ($assigned) {
        Remove-AzRoleAssignment -RoleDefinitionName $Role -Scope $Scope -ObjectId $id
    }
    }
}
